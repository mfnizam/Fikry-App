...
import { MasterService, Siswa } from '../../../../services/master/master.service';
...
export class DetailPage implements OnDestroy{
  ...
  constructor(
    ...
    private modal: ModalService
      ...
      if(this.jenis == 'wali murid'){
        ...
        if(this.dataMaster && this.dataMaster.siswa){
          this.dataMaster.siswa = this.dataMaster.siswa.map(v => {
            let nv = this.master.getValueSiswa().find(e => e._id == v._id);
            return nv? nv : v;
          })
        }
  ...
  hapus(){
    ...
    let dataHapus = this.jenis == 'siswa' || this.jenis == 'wali murid'? this.dataMaster.namaLengkap : this.jenis == 'rekening'? this.dataMaster.noRek : this.dataMaster.title;
    this.modal.showConfirm('Hapus Data ' + this.jenis, 'Apakah anda ingin menghapus data ' + this.jenis + ' <b>"' + dataHapus + '"</b>', ['Batal', 'Hapus']).then(e => {
      if(e){
        if(this.jenis == 'kategori'){
          ...
          this.server.hapusWaliMurid(this.dataMaster._id).then(data => {
            this.modal.hideLoading();
            if(data.success){
              this.master.setDataWaliMurid(this.master.getValueWaliMurid().filter(v => v._id != this.dataMaster._id));
              this.modal.showToast('Berhasil Menghapus Data', 'success');
              setTimeout(_ => {
                this.goBack()
              }, 300)
