...
export class PembayaranPage implements OnDestroy {
	...
	verifikasiBukti(p){
		console.log(p);
		if(!p.buktiPembayaran || p.buktiPembayaran.length < 1) return this.modal.showToast('Gagal, Coba beberapa saat lagi..', 'warning')
		...
		this.modal.showModal({
			...
			if(mdata.role == 'ok' || mdata.role == 'notok'){
				...
				this.server.editPembayaran({idPembayaran: p._id, statusBukti: mdata.role == 'ok'? 1 : 2, idBukti: dataBukti._id}).then(data => {
					...
					console.log(data);
					if(data.success){
						this.modal.showToast('Berhasil menyimpan data verifikasi', 'success');
						if(mdata.role == 'ok'){
						...
						this.modal.showToast('Gagal menyimpan data verifikasi', 'danger');
					}
				}).catch(err => {
					this.modal.hideLoading();
					this.modal.showToast('Gagal menyimpan data verifikasi', 'danger');
